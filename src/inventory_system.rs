...
use super::{WantsToPickupItem, Name, InBackpack, Position, gamelog::GameLog, WantsToUseItem, ProvidesHealing,
            WantsToDropItem, Consumable, CombatStats, InflictsDamage, SufferDamage, Map};
...
impl<'a> System<'a> for ItemUseSystem {
    ...
    fn run(&mut self, data : Self::SystemData) {
        let (player_entity, mut gamelog, map, entities, mut wants_use, names, consumables, healing, mut combat_stats, inflict_damage, mut suffer_damage) = data;
            ...
            match item_heals {
                ...
                Some(healer) => {
                    ...
                    if entity == *player_entity {
                        gamelog.entries.push(format!("Wieclaw zjada {} i odzyskuje {} HP.", names.get(useitem.item).unwrap().name, healer.heal_amount));
                    }
                    used_item = true;
                }
            }
