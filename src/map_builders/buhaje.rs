use super::{BuilderChain, XStart, YStart, AreaStartingPosition, CullUnreachable, VoronoiSpawning,
    AreaEndingPosition, XEnd, YEnd, BspInteriorBuilder, InitialMapBuilder, BuilderMap, TileType};
...
pub fn buhaj_plaza(new_depth: i32, width: i32, height: i32) -> BuilderChain {
    println!("Buhaj plaza builder");
    let mut chain = BuilderChain::new(new_depth, width, height, "Buhajówka - Rynek");
...
impl PlazaMapBuilder {
    ...
    fn portal_park(&mut self, build_data : &mut BuilderMap, voronoi_membership: &[i32], zone: i32, seeds: &[(usize, rltk::Point)]) {
        ...
        build_data.spawn_list.push((idx, "Zloty Zombek".to_string()));
        ...
        altars.iter().for_each(|idx| build_data.spawn_list.push((*idx, "Oltarz".to_string())));

        let demon_spawn = build_data.map.xy_idx(center.x+1, center.y+1);
        build_data.spawn_list.push((demon_spawn, "Wladek Dziadek".to_string()));
    }
