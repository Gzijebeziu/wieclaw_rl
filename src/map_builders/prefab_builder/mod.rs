use super::{MapBuilder, Map, TileType, Position, spawner, super::SHOW_MAPGEN_VISUALIZER, remove_unreachable_areas_returning_most_distant};
...
impl PrefabBuilder {
    ...
    fn char_to_map(&mut self, ch : char, idx: usize) {
        match ch {
            ...
            'g' => { 
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Gnomon".to_string()));
            }
            'G' => {
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Golem Zoledny".to_string()));
            }
            '^' => {
                self.map.tiles[idx] = TileType::Floor;
            ...
            '%' => {
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Surowka".to_string()));
            }
            '∞' => {
                self.map.tiles[idx] = TileType::Floor;
                self.spawn_list.push((idx, "Pasztecik".to_string()));
            }
            _ => {
