...
impl<'a> System<'a> for MeleeCombatSystem {
    ...
    fn run(&mut self, data : Self::SystemData) {
        let (entities, mut log, mut wants_melee, names, combat_stats, mut inflict_damage, melee_power_bonuses, defense_bonuses, equipped, mut particle_builder, positions, hunger_clock) = data;
        ...
        for (entity, wants_melee, name, stats) in (&entities, &wants_melee, &names, &combat_stats).join() {
            if stats.hp > 0 {
                ...
                if target_stats.hp > 0 {
                    ...
                    if damage == 0 {
                        log.entries.push(format!("{} atakuje {} bezskutecznie", &name.name, &target_name.name));
                    } else {
                        log.entries.push(format!("{} atakuje {} za {} obrazen.", &name.name, &target_name.name, damage));
