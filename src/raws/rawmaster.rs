...
use super::{Raws, super::{random_table::RandomTable, Attributes, attr_bonus, npc_hp, mana_at_level}};
...
pub fn spawn_named_item(raws: &RawMaster, new_entity : EntityBuilder, key : &str, pos : SpawnType) -> Option<Entity> {
    if raws.item_index.contains_key(key) {
        ...
        if let Some(armor) = &item_template.armor {
            eb = eb.with(Equippable{ slot: EquipmentSlot::Armor });
            eb = eb.with(DefenseBonus{ defense : armor.defense_bonus });
        }
